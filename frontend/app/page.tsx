...
type GameState = {
  phase: 'setup' | 'step_start' | 'action' | 'game_over'; turn: number; step: number;
...
export default function Home() {
  ...
  async function takeAction() { if (!state || !selection || !current) return; if (pending) { if (pending.kind === 'reveal' && pending.indices.length === pending.count) return void call('/play-magic', { hand_index: pending.handIndex, choices: { indices: pending.indices } }); if (pending.kind === 'target' && pending.target) return void call('/play-magic', { hand_index: pending.handIndex, choices: { target: pending.target } }); if (pending.kind === 'curse' && pending.opponentIndex !== undefined) return void call('/play-magic', { hand_index: pending.handIndex, choices: { opponent_index: pending.opponentIndex } }); if (pending.kind === 'discard' && pending.ownIndex !== undefined && pending.opponentIndex !== undefined) return void call('/play-magic', { hand_index: pending.handIndex, choices: { own_index: pending.ownIndex, opponent_index: pending.opponentIndex } }); if (pending.kind === 'draw-discard' && pending.ownIndices.length === 2) return void call('/open', { index: pending.boardIndex, discard_indices: pending.ownIndices }); setError('Finish the highlighted choice first.'); return; }
