...
export default function Home() {
  ...
  return <main className="game-shell">
    ...
    <div className="table-layout"><aside className="log-panel"><span className="panel-label">WHAT JUST HAPPENED</span><ol>{state.log.slice(-6).reverse().map((entry, index) => <li className={index === 0 ? 'latest' : ''} key={`${entry}-${index}`}><time>{index === 0 ? 'NOW' : `${index} AGO`}</time><p>{entry}</p></li>)}</ol><div className="legend"><span><i className="dot revealed-dot" />Visible, not triggered</span><span><i className="dot opened-dot" />Opened and resolved</span><span><i className="x-mini">X</i>Cannot be opened</span></div></aside>
      <section className="board-wrap"><div className="board-stage"><aside className="board-status" aria-label="Turn and board status"><div className="status-head"><span>TURN</span><strong>{String(state.turn).padStart(2, '0')}</strong><em>{phaseLabel}</em></div><div className="step-order" aria-label={`Step ${Math.min(state.step + 1, 6)} of 6`}>{state.step_order.map((id, index) => <i key={index} className={`${id} ${index === state.step && state.phase !== 'turn_end' ? 'current' : index < state.step || state.phase === 'turn_end' ? 'done' : ''}`}><small>{index + 1}</small><b>{id.toUpperCase()}</b></i>)}</div><div className="player-flow"><div className={state.current_player_id ?? 'neutral'}><span>CURRENT STEP</span><strong>{current?.name ?? (state.phase === 'turn_end' ? 'Review board' : 'Game over')}</strong></div><div className={nextPlayerId ?? 'neutral'}><span>NEXT STEP</span><strong>{nextPlayerId ? state.players.find(player => player.id === nextPlayerId)?.name : '—'}</strong></div></div><div className="composition"><div className="composition-head"><span>CARD TYPE</span><b>THIS</b><b>NEXT</b></div>{compositionKinds.map(kind => <div className={`composition-row kind-${kind.key}`} key={kind.key}><span><i />{kind.label}</span><b>{state.board_distribution.current[kind.key]}</b><b>{state.board_distribution.next[kind.key]}</b></div>)}</div>{state.phase === 'step_start' && <div className="status-decision"><b>STEP-START CHOICES</b><span>{state.step_start.foreteller_count ? `Reveal ${state.step_start.foreteller_count} cards (${stepTargets.length}/${state.step_start.foreteller_count}).` : 'Place optional X barriers.'}</span><div className="barrier-controls">{state.step_start.barrier_player_ids.map(id => <button key={id} className={barriers[id] !== undefined ? 'chosen' : ''} onClick={() => setBarrierOwner(id)}>{id.toUpperCase()} X: {barriers[id] ?? 'SKIP'}</button>)}</div><button disabled={!stepReady || busy} onClick={() => void call('/begin-step', { foreteller_indices: stepTargets, barrier_indices: barriers })}>CONFIRM STEP</button></div>}{state.phase === 'turn_end' && <div className="next-turn"><span>NEXT TURN</span><strong>New 5 × 5 board</strong><small>The final board remains until you continue.</small><button disabled={busy} onClick={() => void call('/continue-turn', {})}>START TURN {state.turn + 1}</button></div>}<div className="board-meta"><b>{selectableCount}</b><span>CAN OPEN</span><button className={resetArmed ? 'reset-armed' : ''} onClick={resetGame}>{resetArmed ? 'CONFIRM RESET' : 'RESET'}</button></div></aside><div className="board-grid">{state.board.map(card => <BoardCard key={card.index} card={card} selected={card.index === selectedBoard} target={targeted.includes(card.index) || Object.values(barriers).includes(card.index)} onClick={() => chooseBoard(card.index)} />)}</div></div></section>
